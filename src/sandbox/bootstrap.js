...
/** Symbol used as a property key to store the event ID of an object */
...
function emitEvent(event) {
    EVENT_PROXY.applyIgnored(undefined, [event], { arguments: { copy: true } })
}
...
/**
 * Get next event ID
 * @param  {any}    [value] Value to taint (if possible)
 * @return {number}         Event ID
 */
function getNextEventId(value) {
    const eventId = nextId++
    if (!isLiteral(value)) {
        value[IdSymbol] = eventId
    }
    return eventId
}
...
function toEventValue(value) {
    ...
    // Logged referenced events
    const eventId = value[IdSymbol]
    if (typeof eventId === 'number') {
        return { ref: eventId }
    }

    // Cannot wrap, probably a mock
    return { unknown: true }
}
...
function onGetOrSetEvent(type, path, value) {
    const wrappedValue = toEventValue(value)
    emitEvent({
        id: getNextEventId(value),
        type,
        path,
        value: wrappedValue,
 ...
 */
function onCallEvent(path, argArray, isConstructor) {
...
function createMock(path, template, thisArg) {
    ...
    const proxy = new Proxy(template, {
        ...
        construct(target, argArray, newTarget) {
            onCallEvent(path, argArray, true)
        ...
        apply(target, realThisArg, argArray) {
            onCallEvent(path, argArray, false)
